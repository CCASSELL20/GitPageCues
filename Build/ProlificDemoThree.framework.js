...
var ASM_CONSTS = {
  3647088: function() {Module['emscripten_get_now_backup'] = performance.now;},  
 3647143: function($0) {performance.now = function() { return $0; };},  
 3647191: function($0) {performance.now = function() { return $0; };},  
 3647239: function() {performance.now = Module['emscripten_get_now_backup'];},  
 3647294: function() {return Module.webglContextAttributes.premultipliedAlpha;},  
 3647355: function() {return Module.webglContextAttributes.preserveDrawingBuffer;},  
 3647419: function() {return Module.webglContextAttributes.powerPreference;}
};
  ...
  function _QT_PostDone() {
      // Send completion message to the parent page (Qualtrics)
      window.parent.postMessage({ type: "UNITY_DONE" }, "*");
    }

  function _RedirectToUrl(urlPtr) {
  ...
  function _SetupBeforeUnload() {
      ...
      window.SendFileToJSForUpload = async function (
        fileName,
        base64Data,
        owner,
        repo,
        ...
        try {
          const sessionPrefix = prefix || "logs";
          const url = `https://api.github.com/repos/${owner}/${repo}/contents/${sessionPrefix}/${fileName}`;
          ...
          const response = await fetch(url, {
            ...
            headers: {
              ...
              "User-Agent": "Unity-WebGL-DataCollector",
          ...
          console.log(`[DataCollector] Upload ${fileName} → ${response.status}`);
        } catch (err) {
          console.error("SendFileToJSForUpload error:", err);
        }
      };
      ...
      const shutdownHandler = function () {
        try {
          if (typeof SendMessage !== "undefined") {
            SendMessage("DataCollector", "PrepareLastExport");
          }
        } catch (e) {
          console.warn("PrepareLastExport failed:", e);
        }
      };
  
      window.addEventListener("pagehide", shutdownHandler, { capture: true });
      window.addEventListener("beforeunload", shutdownHandler, { capture: true });
    }

  function _UploadFileToJS(
      fileNamePtr,
      base64Ptr,
      ownerPtr,
      repoPtr,
      ...
      try {
        ...
        const base64Data = UTF8ToString(base64Ptr);
        const owner = UTF8ToString(ownerPtr);
        const repo = UTF8ToString(repoPtr);
        const branch = UTF8ToString(branchPtr);
        const prefix = UTF8ToString(prefixPtr);
        const token = UTF8ToString(tokenPtr);
  
        if (typeof window.SendFileToJSForUpload === "function") {
          window.SendFileToJSForUpload(
            fileName,
            base64Data,
            owner,
            repo,
          ...
          console.warn("SendFileToJSForUpload() not yet initialized");
        }
      } catch (e) {
        console.error("UploadFileToJS bridge failed:", e);
      }
    }
...
var asmLibraryArg = {
  ...
  "QT_PostDone": _QT_PostDone,
  "RedirectToUrl": _RedirectToUrl,
  "SetupBeforeUnload": _SetupBeforeUnload,
  "UploadFileToJS": _UploadFileToJS,
